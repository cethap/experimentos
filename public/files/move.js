...
function init() {
	...
	socket.on('Gravity', function (data) {
		console.log("Gravedad",data);
		onDocumentMouseMove(data);
		search(data.busca);
	});
...
function search(valu) {
	...
	valu = (typeof valu !== "undefined")?valu:"";
	document.getElementById('q').value = valu;
...
function onImageSearch() {
	...
	for ( var i = 0; i < gImageSearch.results.length; i ++ ) {
		...
		console.log(gImageSearch.results[i]);
