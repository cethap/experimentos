...
window.onload = function() {
	...
	var loop = function() {
		...
		if(Spidery){
			spiderF.thorax.pos.y = -300;
			spiderF.thorax.pos.x = width/2;
		}
	};
	...
	socket.on('Abajo', function (data) {
		console.log("Abajo",data);
		Spidery = 0;
	});


	socket.on('Mueve', function (data) {
		console.log("Mueve",data);
		spiderF.thorax.pos.x = data.left*2;
		spiderF.thorax.pos.y = data.top*2;
	});
